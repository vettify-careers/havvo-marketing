...
export function App({ initialPage }: { initialPage: Page }) {
  ...
  return <><header className="nav"><Link to="home" go={go} className="brand">havvo<span>.</span></Link><nav className={menu ? "open" : ""}>{nav.map(([id,label]) => <Link key={id} to={id} go={go} className={page === id ? "active" : undefined} aria-current={page === id ? "page" : undefined}>{label}</Link>)}</nav><div className="nav-actions"><a className="login" href={APP_URL}>Sign in</a><Link to="contact" go={go} className="primary compact">Join Havvo</Link></div><button className="menu" aria-label="Open menu" aria-expanded={menu} onClick={() => setMenu(!menu)}>☰</button></header>
